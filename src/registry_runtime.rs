use crate::controller_session::issue_mesh_approval;
use crate::network_processes::{
...
use crate::remote_apple_protocol::{AppleRequest, validate_request_envelope};
...
impl LeaseBook {
    ...
    fn try_load(path: PathBuf) -> std::io::Result<Self> {
        let mut leases: Self = load_json_with_fallback(&path)?.unwrap_or_default();
        leases.path = path;
        leases.expire();
        leases.persist()?;
        Ok(leases)
...
impl NetworkArtifacts {
    ...
    fn new(root: PathBuf) -> Self {
        Self::try_new(root).unwrap()
    }

    fn try_new(root: PathBuf) -> std::io::Result<Self> {
        fs::create_dir_all(&root)?;
        let index_path = root.join("index.json");
        let entries = load_json_with_fallback(&index_path)?.unwrap_or_default();
        Ok(Self {
            root,
            index_path,
            entries,
    ...
    while !stopping.load(Ordering::Acquire) {
        ...
        let response = match request {
            Request::Heartbeat { mut host } => {
                ...
                if crate::dashboard::topology::validate_host_snapshot(&host).is_err() {
            ...
            Request::Run { operation } => {
                if let Some(error) = &state.lock().unwrap().recovery_error {
                    ...
                    run(operation, &state, state_path, stopping)
                }
            }
...
fn run(
    ...
    for file in &operation.manifest {
                ...
                .all(|part| matches!(part, Component::Normal(_)))
        {
            return Response {
                accepted: false,
                error: Some("invalid_manifest_path".into()),
                ..response()
            };
        }
    }
...
mod tests {
    ...
    fn restart_reconciliation_rolls_back_a_writer_that_was_never_dispatched() {
        ...
        state.apple_pending.insert(
            "apple-1".into(),
            AppleRequest {
                version: crate::remote_apple_protocol::RemoteProtocolVersion { major: 1, minor: 0 },
                ...
                operation: crate::remote_apple_protocol::AppleOperation::LaunchApp {
