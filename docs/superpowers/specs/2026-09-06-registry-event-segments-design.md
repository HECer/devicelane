...
Implementation inspection on 2026-09-06 identified an additional reuse constraint: `dashboard::audit::windows_private::validate` currently calls `managed_policy::windows_acl_is_restrictive`, whose ACE gate checks `has_policy_write_rights`. That is a policy-integrity check, not a complete owner-private confidentiality check: foreign read-only ACEs are not rejected by that gate. New V2 strict validation must also reject foreign data-read grants, including unsupported allow-ACE forms, and validate the opened object's security information. Reuse of fresh-file creation primitives does not make this existing path-based validator sufficient. This observation does not change the approved private-store invariant and does not authorize a silent permissions repair or claim the new adapter is implemented.
