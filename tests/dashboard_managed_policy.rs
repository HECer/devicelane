use device_development_mesh::dashboard::managed_policy::{
    ManagedPolicyFile, ManagedPolicyLoadError, ManagedPolicyStore, canonical_rules_digest,
};
...
fn signed_owned_bundle_loads_and_untrusted_or_tampered_bundle_is_rejected() {
    ...
    .unwrap();
    secure_file(&path);
    let verified = ManagedPolicyStore::load(&path, &verifier).unwrap();
    ...
    secure_file(&path);
    assert!(matches!(
        ManagedPolicyStore::load(&path, &verifier),
    ...
    secure_file(&path);
    let untrusted =
        SecureTransport::load_or_create(temp.path().join("untrusted"), "untrusted").unwrap();
    assert!(matches!(
        ManagedPolicyStore::load(&path, &untrusted),
        Err(ManagedPolicyLoadError::InvalidSignature)
    ));
}
...
fn permissive_bundle_permissions_are_rejected_before_signature_verification() {
    ...
    assert!(matches!(
        ManagedPolicyStore::load(&path, &identity),
